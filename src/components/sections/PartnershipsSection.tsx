...
const PartnershipsSection = memo(() => {
  ...
  const shuffle = React.useCallback(<T,>(arr: T[]) => {
    const a = arr.slice();
  ...
  const shuffledRow1 = useMemo(() => shuffle(logos), [logos, shuffle]);
  const shuffledRow2 = useMemo(() => shuffle(logos), [logos, shuffle]);
  const shuffledRow3 = useMemo(() => shuffle(logos), [logos, shuffle]);
...
// Provide a display name for the memoized component to satisfy React lint rules
PartnershipsSection.displayName = 'PartnershipsSection';
